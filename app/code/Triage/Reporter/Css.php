...
class Css
{
     ...
     */
    private function reportHelper(): void
    {
        //showMe($this->_analysis['semantics']);
        $selector_count = 0;

        foreach ($this->analysis['selectors'] as $lines) {
        ...
        foreach (['notices', 'warnings', 'errors'] as $trouble_type) {
            ...
            $trouble_reporter = new $syntax_class($this->analysis['syntax'][$trouble_type]);
            $this->all_file_totals['syntax'][$trouble_type] += $trouble_reporter->getCount();
            $trouble_reporter->report($this->show_cows);
        }
        ...
        foreach (['posh', 'microformats', 'microformats2'] as $trouble_type) {
            ...
            $trouble_reporter = new $semantic_class($this->analysis['semantics'][$trouble_type]);
            $this->all_file_totals['semantics'][$trouble_type] += $trouble_reporter->getCount();
    ...
    public function summary(): void
    {
        ...
        echo "+---------------------------+", PHP_EOL;
        echo "|  Syntax                   |", PHP_EOL;
        echo "|  - notices:  " . str_pad(strval($this->all_file_totals['syntax']['notices']), 11, " ") . "  |", PHP_EOL;
        echo "|  - warnings:  " . str_pad(strval($this->all_file_totals['syntax']['warnings']), 10, " ") . "  |"
            . PHP_EOL;
        echo "|  - errors:  " . str_pad(strval($this->all_file_totals['syntax']['errors']), 12, " ") . "  |", PHP_EOL;
        echo "|                           |", PHP_EOL;
        echo "|  Semantics                |", PHP_EOL;
        echo "|  - POSH:  " . str_pad(strval($this->all_file_totals['semantics']['posh']), 14, " ") . "  |", PHP_EOL;
        echo "|  - Microformats:  " . str_pad(strval($this->all_file_totals['semantics']['microformats']), 6, " ")
            . "  |", PHP_EOL;
        echo "|  - Microformats2:  " . str_pad(strval($this->all_file_totals['semantics']['microformats2']), 5, " ")
            . "  |", PHP_EOL;
        echo "+---------------------------+", PHP_EOL;
    }
}
